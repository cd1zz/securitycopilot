...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # First extract the original email content, handling any multipart, TNEF, or message/rfc822 formats
        extracted_content = extract_original_email(raw_input)
        
        if not extracted_content:
            logger.warning("Failed to extract original email, using raw input directly")
            extracted_content = raw_input_str
        
        # Convert to string if it's bytes
        if isinstance(extracted_content, bytes):
            extracted_content_str = extracted_content.decode('utf-8', errors='ignore')
        else:
            extracted_content_str = extracted_content
            
        # Extract subject for Proofpoint detection
        subject = extract_subject_from_raw_input(extracted_content_str)
        logger.info(f"Detected subject: {subject}")
        
        # NOW check if this is a Proofpoint-reported email based on the extracted content
        is_proofpoint = is_proofpoint_reported_email(subject, extracted_content_str)
        ...
        if is_proofpoint:
            ...
            extracted_data = extract_from_proofpoint_email(extracted_content_str)
            ...
            if isinstance(extracted_content, str):
                extracted_content = extracted_content.encode('utf-8')
                
            parsed_email_data = parse_email(extracted_content)
...
def extract_original_email(raw_email: bytes) -> bytes:
    """
    ...
    try:
        ...
        # Store the original raw_email in case we need to fall back to it
        original_raw_email = raw_email

        # Process message/rfc822 parts recursively to extract all content
        if msg.is_multipart():
            for part in msg.walk():
                if part.get_content_type() == 'message/rfc822':
                    ...
                    try:
                        ...
                        if embedded_msg:
                            # Return as bytes if it's not already
                            if isinstance(embedded_msg, EmailMessage):
                                return embedded_msg.as_bytes()
                            return embedded_msg
                    except Exception as e:
                        logger.error(f"Error processing message/rfc822 content: {e}")
                        # Continue to next part rather than stopping completely

        # If no message/rfc822 found, try TNEF handling if available
        if TNEF and msg.get_content_type() == 'application/ms-tnef':
                ...
                if tnef_data:
                    ...
                    for attr in tnef.attributes:
                        if attr.name in ['rtfbody', 'body']:
                            ...
                            try:
                                return attr.data.decode('utf-8', errors='ignore').encode('utf-8')
                            except Exception as e:
                    ...
                    for attachment in tnef.attachments:
                        if attachment.name.lower() == "message.rfc822":
                            ...
                            try:
                                ...
                                body_content = get_body(embedded_msg)
                                return body_content.encode('utf-8') if isinstance(body_content, str) else body_content
                            except Exception as e:
        ...
        # Final fallback: return original raw email
        logger.warning("No valid content found, returning original raw email")
        return original_raw_email
        
    except Exception as e:
        logger.error(f"Error in extract_original_email: {e}")
        return raw_email  # Return original raw email if anything fails
...
def parse_email(raw_email: bytes) -> Dict:
    ...
    try:
        ...
        rfc822_found = False
        # In parse_email function, after checking for message/rfc822 parts:
        if msg.is_multipart():
            for part in msg.walk():
                if part.get_content_type() == 'message/rfc822':
                    ...
                    try:
                        ...
                        if isinstance(embedded_payload, list) and len(embedded_payload) > 0:
                            embedded_msg = embedded_payload[0]
                            # Update headers from embedded message (only if values exist)
                            if embedded_msg.get('Message-ID'):
                                email_headers["message_id"] = embedded_msg.get('Message-ID')
                            if embedded_msg.get('From'):
                                email_headers["sender"] = embedded_msg.get('From')
                            if embedded_msg.get('Return-Path'):
                                email_headers["return_path"] = embedded_msg.get('Return-Path')
                            if embedded_msg.get('To'):
                                email_headers["receiver"] = embedded_msg.get('To')
                            if embedded_msg.get('Subject'):
                                email_headers["subject"] = embedded_msg.get('Subject')
                            if embedded_msg.get('Reply-To'):
                                email_headers["reply_to"] = embedded_msg.get('Reply-To')
                            if embedded_msg.get('Date'):
                                email_headers["date"] = embedded_msg.get('Date')
                            
                            # Also gather the new message's payload for body extraction
...
def get_all_attachments(msg: EmailMessage) -> List[Dict[str, str]]:
    """
    Recursively extracts all attachments from an email, including those in message/rfc822 parts.
    
    Parameters:
    msg (EmailMessage): The email message object.
    
    Returns:
    List[Dict[str, str]]: A list of dictionaries containing attachment information.
    """
    attachments = []
    logger.info("Starting recursive attachment extraction")
    ...
    if msg.is_multipart():
        for part in msg.walk():
            ...
            if content_type == 'message/rfc822':
                logger.info("Processing message/rfc822 part for attachments")
                try:
                    # Get the embedded message
                    embedded_msg = part.get_payload()[0]
                    if embedded_msg:
                        # Recursively process the embedded message for attachments
                        embedded_attachments = get_all_attachments(embedded_msg)
                        attachments.extend(embedded_attachments)
                except Exception as e:
                    logger.error(f"Error processing message/rfc822 attachments: {e}")
