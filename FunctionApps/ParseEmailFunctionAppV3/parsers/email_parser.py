...
from extractors.url_extractor import extract_urls, decode_safelinks, decode_proofpoint_urls, is_url_shortened, batch_expand_urls, dedupe_to_base_urls
    ...
    try:
        ...
        for url in all_urls:
            ...
            if "is_shortened" not in url_obj:
                # Determine if the URL is shortened
            ...
            if "safelinks.protection.outlook.com" in url_str:
                ...
                # Only keep expanded_url if it's a shortened URL 
        ...
        # Apply fixes to URL expansions
        processed_urls = fix_url_expansions(processed_urls)
...
def fix_url_expansions(urls):
    ...
    for url in urls:
        if isinstance(url, dict):
            ...
            # If not a shortened URL, set expanded_url to None
            if not fixed_url.get("is_shortened", False):
                # Only nullify if it was previously set to a placeholder
                if fixed_url.get("expanded_url") in [None, "Not Applicable", fixed_url.get("original_url")]:
                    fixed_url["expanded_url"] = "Not Applicable"
            # If it is shortened but expanded_url is the same as original_url, set to None
...
def parse_email(email_content, depth=0, max_depth=10, container_path=None):
    ...
    try:
        ...
        # Function to fix URL expansion issues
        def fix_url_expansions(urls):
            """
            Fix URL expansion fields to ensure expanded_url is only set for shortened URLs.
